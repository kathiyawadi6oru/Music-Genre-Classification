...
def predict():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            classifier = pickle.load(open('pickle/SVM.pkl', 'rb'))


            #Testing Input Data
            #from collections import Counter
            result=[]
            models = {'SVM' : classifier }#'Catboost':cbc, 'XGBoost':xgbc, 'Gradient Boosting':gbc, 'AdaBoost':abc,  'Linear Regression':lr, 'KNN':cls} #'Random Forest':rfc,
            key_list = list(models.keys())
            val_list = list(models.values())


            for model in val_list:
